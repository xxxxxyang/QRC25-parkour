...
class Go2Ros2Real(Node):
    ...
    def parse_config(self):
        ...
        self.n_proprio = self.cfg["env"]["n_proprio"]
        self.n_depth_latent = self.cfg["env"]["n_scan"]
        self.n_hist_len = self.cfg["env"]["history_len"]
    ...
    def start_ros_handlers(self):
        ...
        if self.depth_data_topic is not None:
    ...
    def _depth_data_callback(self, msg):
        resolution = self.cfg["depth"].get(
            "original",
            [87, 58]
        )   # (W, H)
        self.depth_data = torch.tensor(msg.data, dtype=torch.float32).reshape(1, resolution[1], resolution[0]).to(self.model_device)
    ...
    def get_proprio(self):
        ...
        vx = xyyaw_commands[0, 0].item()
        commands = torch.tensor([[0, 0, vx]], device= self.model_device, dtype= torch.float32)  # only forward velocity command
